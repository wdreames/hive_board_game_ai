...
def make_choice(board_manager, description, prompt, choices):
...
def _save_and_exit(board_manager, last_description, last_prompt, last_choices):
    ...
    if confirm_exit != 'Yes':
        return make_choice(board_manager, last_description, last_prompt, last_choices)
    ...
    if file_name:
        board_manager.save_state(file_name)
    else:
